...
const fetchDealsEvents = async ({
  ...
  const response = await fetch(
    `https://api.ebay.com/buy/deal/v1/event?${params}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
        "X-EBAY-C-MARKETPLACE-ID": site,
...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetchDealsEvents({
    ...
    if (response.status !== 200) {
      return new Response(`Failed to fetch ebay-deals-events for ${site}`, {
        status: 500,
        headers: { "Content-Type": "text/plain" },
      });
    }
    ...
    console.error(`Error fetching ebay-deals-events for ${site}:`, error);
    return new Response(`Failed to fetch ebay-deals-events for ${site}`, {
      status: 500,
      headers: { "Content-Type": "text/plain" },
    });
