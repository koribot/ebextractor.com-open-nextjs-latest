...
const fetchDeals = async ({
  ...
  const response = await fetch(
    `https://api.ebay.com/buy/deal/v1/deal_item?${params.toString()}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
        "X-EBAY-C-MARKETPLACE-ID": site,
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (response.status !== 200) {
      return new Response(`Failed to fetch ebay-default-deals-v2 third-party for ${site}`, {
    ...
    return new Response(`Failed to fetch ebay-default-deals-v for ${site}`, {
      status: 500,
      headers: { "Content-Type": "text/plain" },
    });
  }
